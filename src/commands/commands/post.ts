...
async function PostCopy(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
        ...
        const perChFrom = testPermisos(meFrom, "chsee|reedMsg");
        ...
        const perChTo = testPermisos(meTo, "chsee|sendmsg|addlink|addfiles");
        if (perChTo.some(p => p.includes("❌"))) {
...
async function PostEdit(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
        ...
        const meTo = targetChannel.permissionsFor(interaction.guild!.members.me!);
        const perChTo = testPermisos(meTo, "chsee|msgManager|reedMsg|addlink|addfiles");
        if (perChTo.some(p => p.includes("❌"))) {
            await interaction.editReply({ content: i18next.t("common:Errores.missing_permissions", { a1: `<#${targetChannel.id}>`, a2: perChTo.join("\n") }) });
            return;
        }
...
async function PostReply(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
        ...
        const perChTo = testPermisos(meTo, "chsee|reedMsg|sendmsg|addlink|addfiles");
        if (perChTo.some(p => p.includes("❌"))) {
