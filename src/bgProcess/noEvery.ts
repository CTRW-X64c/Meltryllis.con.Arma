...
export async function handleNoEveryoneCommand(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
        ...
        const im = guild.members.me!;
        const serPrm = testPermisos(im.permissions, "viewCh|readMsg|msgManager");
        ...
        const x = testPermisos(im.permissions, "moderateMembers"); if (x.some(p => p.includes("❌"))) { castigo = false; desc += i18next.t("commands:noeveryone.interacciones.embMaker_desc_2", { a1: x[0] }) };
        ...
        if (castigo) {
            const rChunk = await topRol(guild, im, "moderateMembers");
