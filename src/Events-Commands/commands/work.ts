...
export async function registerWorkCommand(): Promise<SlashCommandBuilder[]> {
  const workCommand = new SlashCommandBuilder()
    ...
    .setDescription(i18next.t("work:slashBuilder.description"))
    .addStringOption((option) => option
        .setName("mode").setDescription(i18next.t("work:slashBuilder.state_description"))
        .setRequired(true)
        .addChoices({ name: "workhere", value: "where" }, { name: "replybots", value: "rbots" })
      )
    .addStringOption((option) => option
        .setName("turn").setDescription(i18next.t("work:slashBuilder.mode_description"))
        .setRequired(true)
...
export async function handleWorkCommand(interaction: ChatInputCommandInteraction): Promise<void> {
  try {
    ...
    switch (state) {
      ...
      default:
        await interaction.reply({
          content: i18next.t("work:interacciones.command_error"),
          flags: MessageFlags.Ephemeral,
        });
        break;
    }
...
async function where(interaction: ChatInputCommandInteraction): Promise<void> {
  try {
    ...
    if (!guildId || !channelId) {
      throw new Error(i18next.t("work:interacciones.error_identifying_channel"));
    }
    ...
    if (isEnabled) {
      if (currentConfig?.enabled === true) {
        response = i18next.t("work:interacciones.already_on");
      } else {
        setChannelConfig(guildId, channelId, { enabled: true, replyBots: currentConfig?.replyBots ?? false });
        response = i18next.t("work:interacciones.on_success") || "Bueno, estoy de regreso 😎";
      }
    } else {
      if (currentConfig?.enabled === false) {
        response = i18next.t("work:interacciones.already_off");
      } else {
        setChannelConfig(guildId, channelId, { enabled: false, replyBots: currentConfig?.replyBots ?? false });
        response = i18next.t("work:interacciones.off_success");
      }
    }
...
async function replybots(interaction: ChatInputCommandInteraction): Promise<void> {
  try {
    ...
    if (!guildId || !channelId) {
      throw new Error(i18next.t("work:interaccionesRole.identifying_server"));
    }
    ...
    if (isReplyBots) {
      if (currentConfig?.replyBots === true) {
        response = i18next.t("work:interaccionesRole.already_on");
      } else {
        setChannelConfig(guildId, channelId, { enabled: currentConfig?.enabled ?? true, replyBots: true });
        response = i18next.t("work:interaccionesRole.on_success");
      }
    } else {
      if (currentConfig?.replyBots === false) {
        response = i18next.t("work:interaccionesRole.already_off");
      } else {
        setChannelConfig(guildId, channelId, { enabled: currentConfig?.enabled ?? true, replyBots: false });
        response = i18next.t("work:interaccionesRole.off_success");
      }
    }
